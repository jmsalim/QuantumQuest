...
  resetGame();
  
  // Initialize the chaotic vortex points
...
        strokeWeight(1);
        noFill();
        rect(boxX, boxY, boxW + 10, boxH + 10, 10);
        rectMode(CORNER);
...
        textFont(titleFont);
        textSize(30);
        text(notificationMessage.title, boxX, boxY - 5); // Centered by boxX 
...
    p.x += map(noiseX, 0, 1, -1.5, 1.5);
    p.y += map(noiseY, 0, 1, -1.5, 1.5);
...
    
    fill(c, 150); 
    ellipse(p.x, p.y, 4, 4); // Draw the star/particle
  }
}
...
            let offset = map(noise(y * 0.1, frameCount * 0.1), 0, 1, -intensity * 10, intensity * 10);
            
            // random horizontal line segments to create noise
            fill(255, 50, 50, intensity * 50); // Red static
            rect(offset, y, width / 2, 2); 
            fill(50, 50, 255, intensity * 50); // Blue static
            rect(width - offset - width / 2, y + 2, width / 2, 2);
        }
...
    text("[Q] West Wing Stabilizer", width / 2 - 200, height - 200);
    text("[W] East Wing Stabilizer", width / 2 + 200, height - 200);
    text("[E] Armory", width / 2 - 200, height - 150);
    text("[R] Medical Chamber", width / 2 + 200, height - 150);

    // Paths to Initial Equipment
    text("[Z] Shield Vault", width / 2 - 200, height - 100); 
    text("[C] Capacitor Incubator", width / 2 + 200, height - 100); 

    // Final Chamber Exit
    fill(200, 255, 200);
    text("[X] Proceed to QUANTUM BREECH", width / 2, height - 50);

    //  Tutorial Access Prompt
    fill(100, 255, 100);
    textSize(16);
    textAlign(LEFT);
    text("[T] Re-read Tutorial", 20, height - 20);
    textAlign(CENTER);
}
...
        textSize(20);
        text("Please wait...", width / 2, height / 2 + 50);
        return;
    }
    
    textFont(gameFont); 
...
        // The image is placed high up (y=150) for equipment/stabilizers
        image(img, width / 2 - 100, 150, 200, 200); 
    } else {
        // Placeholder rectangle if image fails to load
        fill(50, 50, 50); 
        rect(width / 2 - 100, 150, 200, 200, 10);
        fill(255);
        textFont(gameFont);
        textSize(16);
        text("Asset Loading Error", width / 2, 250);
        textSize(24);
    }
}
...
  textSize(50);
  
  text("THE QUANTUM PARADOX", width / 2, 80);
  
  textFont(gameFont); // Switch to game font for body text
  textSize(25); // Slightly larger font for better readability in the lore block
  textLeading(30); // Increased line spacing

  // Define the central text block area (70% width, starting at Y=180)
  const loreX = width / 6.5;
  const loreY = 350;
  const loreW = width * 0.7; 
  const loreH = height * 0.5;
  fill(255, 219, 88);
  const loreText = 
...
  // We specify the center point (loreX, loreY) and the max width/height.
  text(loreText, loreX, loreY, loreW); 
  fill(0, 160, 255); 
  textSize(24);
  text("Press [SPACE] to begin", width / 2, height - 50);
}
...
  textFont(titleFont);
  textSize(40);
  text("CHOOSE YOUR HERO", width / 2, 80); 
  
  textFont(gameFont);
  textSize(20);
...
  const boxW = 250;
  const boxH = 380;
  const startX = width / 2 - boxW * 1.65;
  const boxY = height / 2 - 140;
  const spacing = boxW + 50;
...
      image(soldierImg, startX + 25, boxY + 60, 200, 200);
  } else {
      fill(100); noStroke(); rect(startX + 25, boxY + 60, 200, 200);
      fill(255); textSize(16); text("Image N/A", startX + boxW / 2, boxY + 160);
  }
  
  // Draw Text (Set Fill and Text Size explicitly for the character block)
  fill(255); textSize(24); text(CHARACTERS.SOLDIER.name, startX + boxW / 2, boxY + 30);
  textSize(16); text(`STR: ${CHARACTERS.SOLDIER.STR}, INT: ${CHARACTERS.SOLDIER.INT}`, startX + boxW / 2, boxY + 310);
textSize(20); text("Press [S]", startX + boxW / 2, boxY + boxH - 20);
  
  // Character 2: Doctor (Dr. Kaelen Varr)
  // Draw Outline
  noFill(); 
  rect(startX + spacing, boxY, boxW, boxH, 10); 

  // Draw Image/Placeholder
  if (doctorImg && doctorImg.width > 0) {
      image(doctorImg, startX + spacing + 25, boxY + 60, 200, 200);
  } else {
      fill(100); noStroke(); rect(startX + spacing + 25, boxY + 60, 200, 200);
      fill(255); textSize(16); text("Image N/A", startX + spacing + boxW / 2, boxY + 160);
  }

  // Draw Text
  fill(255); textSize(24); text(CHARACTERS.DOCTOR.name, startX + spacing + boxW / 2, boxY + 30);
  textSize(16); text(`STR: ${CHARACTERS.DOCTOR.STR}, INT: ${CHARACTERS.DOCTOR.INT}`, startX + spacing + boxW / 2, boxY + 310); textSize(20); text("Press [D]", startX + spacing + boxW / 2, boxY + boxH - 20);

  // Character 3: Robot (UNIT 734)
  // Draw Outline
  noFill(); 
  rect(startX + spacing * 2, boxY, boxW, boxH, 10); 

  // Draw Image/Placeholder
  if (robotImg && robotImg.width > 0) {
      image(robotImg, startX + spacing * 2 + 25, boxY + 60, 200, 200);
  } else {
      fill(100); noStroke(); rect(startX + spacing * 2 + 25, boxY + 60, 200, 200);
      fill(255); textSize(16); text("Image N/A", startX + spacing * 2 + boxW / 2, boxY + 160);
  }

  // Draw Text
  fill(255); textSize(24); text(CHARACTERS.ROBOT.name, startX + spacing * 2 + boxW / 2, boxY + 30);
  textSize(16); text(`STR: ${CHARACTERS.ROBOT.STR}, INT: ${CHARACTERS.ROBOT.INT}`, startX + spacing * 2 + boxW / 2, boxY + 310); textSize(20); text("Press [B]", startX + spacing * 2 + boxW / 2, boxY + boxH - 20);
}
...
  
  text(narrationText, width / 5, height / 2 - 200, width * 0.6, height * 0.5); 

  textSize(24);
  text("Press [SPACE] to continue...", width / 2, height - 50); // Changed prompt
}
...
    text(explanationText, width / 5, height / 2 - 200, width * 0.6, height * 0.6);
    
    fill(0, 160, 255);
    textSize(24);
    text("Press [SPACE] for more...", width / 2, height - 50);
}
...
    fill(0, 160, 255);
    textSize(24);
    text("Press [SPACE] to enter the Quantum Labyrinth...", width / 2, height - 50);
}
...
    fill(0, 255, 255);
    textSize(45);
    text("QUANTUM STABILIZATION SEQUENCE", width / 2, 80);
...
        
        // Time Bar (Visual)
        let barWidth = map(sequenceTimer, 0, sequenceMaxTime, 0, 700, true);
        
        // Color changes from Green to Red
        let barColor = lerpColor(color(255, 0, 0), color(0, 255, 0), sequenceTimer / sequenceMaxTime);
        fill(barColor); 
        rect(width / 2 - 350, height / 2 - 50, barWidth, 40);
        
        // Critical Key Prompt
        fill(255);
        textSize(24);
        text("CRITICAL: PRESS THE TARGET KEY NOW!", width / 2, 200);
        
        fill(255, 255, 0);
        textSize(100);
        text(sequenceTargetKey, width / 2, height / 2 + 100);
        
        fill(255);
        textSize(40);
        text(timeRemaining + "s", width / 2, height / 2 - 100);
        
        // Sequence Index
        fill(0, 255, 100);
        textSize(20);
        text(`STABILIZER STEP: ${sequenceStep + 1} / ${SEQUENCE_KEYS.length}`, width / 2, height - 100);
        
...
        text(actionDetails.prompt, width / 2, height / 2 - 150);
        fill(255, 200, 0); textSize(24);
        text(`COST: ${actionDetails.cost}`, width / 2, height / 2 - 50);
...
    fill(100, 255, 100); text("Press [SPACE] to CONFIRM.", width / 2, height - 150);
    fill(255, 100, 100); text("Press [N] to CANCEL.", width / 2, height - 100);
}
...
 
 textFont(titleFont);
 fill(255); textSize(32); text("Chrono-Cache (Shield)", width / 2, 80);
 
 textFont(gameFont);
 textSize(22);
 
...
      
      fill(0, 255, 255);
      text("The Shield is guarded by a complex temporal lock (High Inteligence).", width / 2, 370);
      text("Press [SPACE] to attempt the INTELLECT check.", width / 2, 420);
    } else {
...
  
  textFont(titleFont);
  fill(255); textSize(32); text("Capacitor Aparatus", width / 2, 80);
  
  textFont(gameFont);
  textSize(22);
  handleRandomEncounter();
...
      
      text("The Capacitor is surrounded by volatile energy.", width / 2, 370);
      text("Press [SPACE] to attempt the INTELLECT check.", width / 2, 420);
    } else {
...
  
  textFont(titleFont);
  fill(255); textSize(32); text("WEST WING", width / 2, 80);
  
  textFont(gameFont);
  textSize(22);
  handleRandomEncounter();
...
      
      fill(255);
      text("Retrieving this stabilizer requires brute force.", width / 2, 370);
      text("Press [SPACE] to attempt the STRENGTH check.", width / 2, 420);
    } else {
...
  
  textFont(titleFont);
  fill(255); textSize(32); text("EAST WING", width / 2, 80);
  
  textFont(gameFont);
  textSize(22);
  handleRandomEncounter();
...
      
      fill(255);
      text("This devices lock requires precise manipulation.", width / 2, 370);
      text("Press [SPACE] to attempt a INTELLECT check.", width / 2, 420);
    } else {
...
  
  textFont(titleFont);
  fill(255); textSize(32); text("ARMORY", width / 2, 80);
  
  textFont(gameFont);
  textSize(22);
  handleRandomEncounter();
...
      
      fill(255);
      text("A damage amplifier is secured by a delicate quantum lock.", width / 2, 370);
      text("Press [SPACE] to attempt the INTELLECT check.", width / 2, 420);
    } else {
...
  
  textFont(titleFont);
  fill(255); textSize(32); text("Medical Terminal)", width / 2, 80);
  
  textFont(gameFont);
  textSize(22);
  handleRandomEncounter();
...
        fill(100, 255, 100);
        text("You have reached the medical terminal. You may trade a stabilizer for a full recovery.", width / 2, 370);
        text("Press [SPACE] to sacrifice ONE stabilizer to recover ALL of your health.", width / 2, 420);
    } else if (playerHealth < 10 && collectedStabilizers === 0) {
        fill(255, 150, 150);
        text("You need at least 1 Stabilizer to activate the Anchor Point for healing.", width / 2, 370);
    } else {
        text("Your health is already at maximum capacity (10 HP).", width / 2, height / 2 + 10);
    }
...
    
    textFont(titleFont);
    fill(255); textSize(40); text("FINAL CHAMBER", width / 2, 80);
    
    textFont(gameFont);
    textSize(22);
...
    fill(255, 200, 0);
    text("Press [N] to return to the Labyrinth.", width / 2, height - 90);
  } else {
    fill(255, 200, 0);
    text("All Stabilizers collected. The Reality Engine hums ominously.", width / 2, height / 2 - 50);
...
    textFont(bossFont);
    fill(255, 50, 200);
    textSize(50); // Increased text size for boss titles
    text("THE FINAL PARADOX", width / 2, 80);
    
    // Draw the Paradox Entity Image
    if (paradoxEntityImg && paradoxEntityImg.width > 0) {
        //  Image size increased to 300x300, and moved slightly up (Y=120)
        image(paradoxEntityImg, width / 2 - 150, 120, 300, 300); 
    } else {
        fill(50, 50, 50); 
        rect(width / 2 - 150, 120, 300, 300, 10);
        fill(255);
        textFont(gameFont);
        textSize(16);
        text("Asset Loading Error", width / 2, 270);
        textSize(24);
    }
...
    text(hypeText, width / 5, 450, width * 0.6, 100); 
...
    fill(255, 200, 50);
    textSize(28);
    text("Press [SPACE] to fight!", width / 2, height - 100);
}
...
  
  textFont(titleFont);
  fill(255, 255, 0); textSize(32); text("A TEMPORAL MERCHANT APPEARS", width / 2, 100);
  
  textFont(gameFont);
  textSize(24);
  text("Merchant: 'I have no need for your timeline's trinkets, traveler.'", width / 2, height / 2 - 50);
  text("Press [N] to Continue and ignore the suspicious figure.", width / 2, height / 2 + 100);
}
...
  text("RANDOM ENCOUNTER: MINOR PARADOX WRAITH", width / 2, 100);
  
  textFont(gameFont);
  textSize(24);
  
...
            image(vortexWraithImg, width / 2 - 100, 180, 200, 200); 
        } else {
            fill(50, 50, 50); rect(width / 2 - 100, 180, 200, 200, 10);
            fill(255); textSize(16); text("Wraith Image Missing", width / 2, 280);
        }
        
        // Minor Enemy Health Bar
        fill(50, 50, 50); rect(width / 2 - 100, 145, 200, 15);
        fill(255, 100, 100);
        let wraithHealthWidth = map(minorEnemyHealth, 0, 3, 0, 200);
        rect(width / 2 - 100, 145, wraithHealthWidth, 15);
        fill(255); textSize(16); text("WRAITH HP: " + max(0, minorEnemyHealth), width / 2, 152);
        
        if (!battleStarted) {
            // PHASE 0: Tutorial/Start Phase (non-hostile)
            fill(255, 255, 255);
            textSize(24);
            text("A minor vortex wraith appears, distorting the timelines!", width / 2, height / 2 - 100);
            fill(255, 200, 0);
            text("Press [SPACE] to engage the wraith and start combat!", width / 2, height / 2);
            
        } else {
            // PHASE 1: Combat Phase (Hostile/Damage clock active)
            text("VORTEX WRAITH ATTACK!", width / 2, height / 2 - 100);
            
...
            text("Press [SPACE] to attack!", width / 2, height / 2 + 50);
        }
  } else {
    text("The wraith is gone.", width / 2, height / 2);
  }
  fill(255); text("Press [N] to continue path.", width / 2, height / 2 + 100);
}
...
    
    textFont(bossFont);
    fill(255); textSize(50); // Increased text size for boss titles
    text("BOSS: PARADOX ENTITY", width / 2, 80);
  
  // Draw the Paradox Entity Image slightly lower to clear the health bar
  if (paradoxEntityImg && paradoxEntityImg.width > 0) {
      image(paradoxEntityImg, width / 2 - 100, 220, 200, 200); 
  } else {
      fill(50, 50, 50); 
      rect(width / 2 - 100, 220, 200, 200, 10);
      fill(255);
      textFont(gameFont);
      textSize(16);
      text("Asset Loading Error", width / 2, 320);
      textSize(40);
  }
  
...
  
  // BOSS Health Bar
  noFill(); rect(width / 2 - 150, 160, 300, 30);
  fill(255, 0, 0);
  let bossHealthWidth = map(bossHealth, 0, 35, 0, 300);
  rect(width / 2 - 150, 160, bossHealthWidth, 30);
  textFont(gameFont);
  textSize(22); text("BOSS HEALTH: " + max(0, bossHealth), width / 2, 175);
...
        textSize(24);
        text(`Press [SPACE] to fire a Quantum Blast! (Deals ${baseDmg} damage)`, width / 2, height / 2 + 100);
    }
    
    // Universal Flux Capacitor Action
    if (hasFluxCapacitor && !capacitorUsed) {
      fill(100, 255, 100);
      text("Press [H] to use the FLUX-CAPACITOR (Heal 5 HP once).", width / 2, height - 50);
    }
}
...
  textFont(titleFont);
  textSize(40);
  text("SUCCESS! REALITY RESTORED!", width / 2, height / 2 - 50);
  
  textFont(gameFont);
  textSize(24);
  if (selectedCharacter) {
    text(selectedCharacter.name + " saved the universe!", width / 2, height / 2 + 20);
  } else {
    text("You saved the universe!", width / 2, height / 2 + 20);
  }
  textSize(22); text("Press [R] to reset and play again.", width / 2, height / 2 + 80);
}
...
  textFont(titleFont);
  textSize(40);
  text("FAILURE! PARADOX CONSUMED YOU!", width / 2, height / 2 - 50);
  
  textFont(gameFont);
  textSize(24);
  if (!hasQuantumShield && gameState === BOSS_ROOM) {
    text("You lacked the  QUANTUM SHIELD ! Instant Paradox Collapse.", width / 2, height / 2 + 20);
  } else if (playerHealth <= 0) {
    text("Your defenses failed! " + (selectedCharacter ? selectedCharacter.name : "You") + " ran out of health.", width / 2, height / 2 + 20);
  } else {
    text("The Reality Engine is lost forever.", width / 2, height / 2 + 20);
  }
  textSize(22); text("Press [R] to reset and try again.", width / 2, height / 2 + 80);
}
...
  text("Hero: " + selectedCharacter.name, 20, 15);
  text("Stabilizers: " + collectedStabilizers, 20, statusY + 15);
  
  // Equipment Status (Center)
  textAlign(CENTER, CENTER);  
  text("EQUIPMENT:", width/2 - 50, 15);
  
  const equipX = width/2 + 20;
  fill(hasQuantumShield ? color(0, 255, 255) : color(100)); text("S", equipX, 15);
  fill(hasTemporalAmplifier ? color(255, 255, 0) : color(100)); text("A", equipX + 30, 15);
  fill(hasFluxCapacitor && !capacitorUsed ? color(255, 100, 255) : color(100)); text("C", equipX + 60, 15);


  // Player Health Bar (Right Side)
  textAlign(RIGHT, CENTER);
  fill(100, 100, 100); rect(rightOffset, 5, 180, 20);
  let healthColor = lerpColor(color(255, 0, 0), color(0, 255, 0), playerHealth / 10);
  fill(healthColor);
  let playerHealthWidth = map(playerHealth, 0, 10, 0, 180);
  rect(rightOffset, 5, playerHealthWidth, 20);
  fill(0); text("HEALTH: " + max(0, playerHealth), rightOffset + 135, 15);

  // Character Stats (Below Health)
  textAlign(RIGHT, CENTER);
  textSize(14);
  fill(100, 150, 255); text(`STR: ${selectedCharacter.STR}`, rightOffset + 180, statusY + 10);
  fill(255, 150, 100); text(`INT: ${selectedCharacter.INT}`, rightOffset + 180, statusY + 30);
...
                bossHealth -= damage;
                fill(255, 0, 0); ellipse(width / 2, 180, 50, 50); 
